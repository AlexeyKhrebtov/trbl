@extends('layouts.app')

@section('content')
    <div class="container">
        <h1>{{ $cabinet->title }}</h1>

        <div class="row">
            <div class="col-sm-4">
                <p class="lead">Шкаф</p>
            </div>
            <div class="col-sm-4">
                <a href="{{ route('cabinets.index') }}" class="btn btn-outline-info mb-2" role="button"><i class="fas fa-undo"></i> Вернуться к списку шкафов</a>
            </div>
            <div class="col-sm-4">
                <a href="{{ route('cabinets.edit', $cabinet->id) }}" class="btn btn-outline-dark mb-2"  role="button"><i class="fas fa-edit"></i> Редактировать шкаф</a>
            </div>
        </div>

        <div class="shadow-sm p-3 mb-5 bg-white rounded">
            <dl class="row">
                <dt class="col-sm-3">Номер</dt>
                <dd class="col-sm-9">{{ $cabinet->title }}</dd>
                <dt class="col-sm-3">Местоположение</dt>
                <dd class="col-sm-9">{{ $cabinet->location_km }} км, {{ $cabinet->location_piket }} пикет</dd>
                <dt class="col-sm-3">ОПО</dt>
                <dd class="col-sm-9"><a href="{{ route('sectors.show', $cabinet->sector->id)}}">{{ $cabinet->sector->title }} &nbsp; <i class="fas fa-external-link-alt"></i></a></dd>
                <dt class="col-sm-3">Комментарий</dt>
                <dd class="col-sm-9">{!! nl2br(e($cabinet->comment)) !!}</dd>
                <dt class="col-sm-3">Координаты</dt>
                <dd class="col-sm-9">
                    <div class="input-group input-group-sm mb-2">
                        <div class="input-group-prepend">
                            <div class="input-group-text">
                                <i class="fas fa-map-marker-alt"></i>
                            </div>
                        </div>
                        <input class="form-control" type="text" readonly value="{{ $cabinet->lat }}, {{ $cabinet->lng }}">
                        <div class="input-group-append">
                            <button class="btn btn-outline-secondary" type="button">copy</button>
                        </div>
                    </div>
                    <p>Открыть в <a href="yandexnavi://build_route_on_map?lat_to={{  $cabinet->lat }}&lon_to={{ $cabinet->lng }}">Яндекс.Навигатор <img src="data:image/svg+xml;base64," style="height:22px;"></a> или показать на <a href="http://maps.yandex.ru/?text={{ $cabinet->lat }}N,{{ $cabinet->lng }}E" target="_blank">Яндекс.Карты <img src="data:image/svg+xml;base64," style="height:22px;"></a></p>

                </dd>
            </dl>

            <form-map v-bind:coords="{lat: {{$cabinet->lat}}, lng: {{$cabinet->lng}}}"></form-map>

        </div>

        <h2>Камеры <i class="fas fa-camera"></i></h2>

        <div class="list-group mb-5">
        @forelse ($cabinet->cameras as $camera)
            <a href="{{ route('cameras.show', $camera->id) }}" class="list-group-item list-group-item-action">
                <div class="d-flex w-100 justify-content-between">
                    <h5 class="mb-1">{{ $camera->title }}</h5>
                    <small class="text-muted">{{ $camera->location_km }} км, {{ $camera->location_piket }} пикет</small>
                </div>
                <p class="mb-1">{{ $camera->comment }}</p>
                <small class="text-muted">#{{ $camera->id }}</small>
            </a>
        @empty
                <p>нет привязанных камер</p>
        @endforelse
        </div>

        <div class="pull-right">
            <a href="#" class="btn btn-danger disabled">Удалить шкаф</a>
        </div>
    </div>
@endsection
